...
export default function PostCard({ post, onDelete, className }: PostCardProps) {
  ...
  const handleLike = () => {
    ...
    const nowLiked = togglePostLike(post.id, userId);
    setIsLiked(nowLiked);
    setLikeCount(prev => nowLiked ? prev + 1 : Math.max(0, prev - 1));

    if (nowLiked && safeUser.id !== userId) {
      saveNotification({
        id: `like-${post.id}-${userId}-${Date.now()}`,
        type: "like",
        fromUserId: userId,
        fromUser: currentUser ?? undefined,
        toUserId: safeUser.id,
        postId: post.id,
  ...
  const handleAddComment = (content: string) => {
    ...
    if (safeUser.id !== userId) {
      saveNotification({
        id: `comment-${post.id}-${userId}-${Date.now()}`,
        type: "comment",
        fromUserId: userId,
        fromUser: currentUser ?? undefined,
        toUserId: safeUser.id,
