...
import { Home, Compass, Bell, User, TrendingUp, Plus, Heart, Music } from "lucide-react";
    ...
    <aside className="h-full p-6 flex flex-col">
      ...
      <div className="mb-8 animate-fade-up delay-200">
        ...
        <div className="space-y-1">
          {[
            { to: "/liked-songs", icon: Heart, label: "Liked Songs", count: isConnected ? likedCount : null },
            { to: "/playlists", icon: Music, label: "My Playlists", count: isConnected ? playlistCount : null },
          ].map(({ to, icon: Icon, label, count }) => (
            <Link
              key={to}
              to={to}
              className="group flex items-center justify-between px-3 py-2 rounded-xl text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-white/30 dark:hover:bg-gray-800/30 transition-all duration-300"
            >
              <div className="flex items-center gap-3">
